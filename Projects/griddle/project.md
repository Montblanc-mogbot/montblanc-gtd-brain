...
- [ ] #nextaction Validation UX prep, slice 6: add a pure helper that turns grouped validation issues into compact section summaries (counts + stable labels) for a future panel, plus focused unit tests and minimal doc notes if needed. Keep it domain-only and additive; no UI rendering yet. Review any changes for anything that might introduce bugs, especially inventing UI semantics that the current interaction model cannot support.
- [ ] #nextaction Validation UX prep, slice 7: extend the manual validation workflow docs with a tiny checkpoint describing how grouped issue summaries should map back to Entry vs Full Records navigation without mutating filters or views. Keep it doc-only, bounded, and aligned with the existing targeting rules.
