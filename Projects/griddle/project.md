...
- [ ] #nextaction Extract `src/App.tsx` workspace selection/panel transitions into a first small helper layer with named functions for the already-repeated reset/open paths (`clearWorkspaceSelection`, `openFullRecordsFromBulk`, `openEntryFromSelection`, etc.) without changing behavior yet. Keep the change narrowly scoped to obvious duplication removal and state-transition naming. Review any changes for anything that might introduce bugs, especially invalid state combinations, stale selection context, and pointer-up/pointer-down races.
- [ ] #nextaction Add targeted regression coverage for the first helper extraction pass: at minimum, document and/or codify smoke checks for clear-selection behavior, same-cell reselection, bulk → full-records open, and full-records close back to entry. Review any changes for anything that might introduce bugs, especially false confidence from incomplete checks or missed transition paths.
- [ ] #nextaction Refactor the remaining `src/App.tsx` interaction handling so panel/selection transition decisions are centralized behind a reducer-backed controller or a second-pass transition helper once the first helper layer is stable. Review any changes for anything that might introduce bugs, especially keyboard navigation regressions, stale selection context after dataset changes, and pointer gesture race conditions.
