...
- [ ] Workflow rule (not a heartbeat task): Continue all Griddle implementation work on branch `stabilization-pass-1` against the `fork` remote first; only propose upstream merge/promotion after validation on the integration fork.
