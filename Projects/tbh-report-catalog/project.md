...
- [x] #nextaction Add a **GLDT validation report** for month-by-month comparison to trusted historical reports: output **trans_amt** by **cost_center + descr_y + acct_code + descr_x**. — DONE: matched the current Python report contract by adding raw extractor support for `gldt`, `acct`, and `cost`; added `GldtValidationBuilder` that filters out `qty_trans_flag == TRUE`, joins account/cost descriptions, and outputs `trans_amt` plus `reversed_amount`; and wired the analytics artifact `GldtValidation_ByCostCenterAccount.csv` into `src/Tbh.ReportCatalog/Pipeline/PipelineRunner.cs`. Added focused coverage in `src/Tbh.ReportCatalog.Tests/Analytics/GldtValidationBuilderTests.cs`. Validation: `dotnet build TbhReportCatalog.sln`, `dotnet test TbhReportCatalog.sln` (24 tests), and `dotnet run --project src/Tbh.ReportCatalog -- --month 2025-01` all passed on 2026-03-30. Current dummy DB caveat: `data/command_alkon_dummy.db` does not contain `gldt`, `acct`, or `cost`, so the emitted CSV is currently header-only on dummy data; live data or a refreshed dummy DB is needed to validate real row output.
- [x] #nextaction Add an **STXD sales tax validation report** for month-by-month comparison to trusted historical reports: output **taxble_amt + non_taxble_amt + tax_amt** by **tax_code + descr**. — DONE: added raw extractor support for `stxd` + `taxc` in the Command extractor interface/implementations, added `SalesTaxValidationBuilder` + coverage in `src/Tbh.ReportCatalog.Tests/Analytics/SalesTaxValidationBuilderTests.cs`, and wired a new analytics artifact `SalesTaxValidation_ByTaxCode.csv` into `src/Tbh.ReportCatalog/Pipeline/PipelineRunner.cs`. Validation: `dotnet build TbhReportCatalog.sln`, `dotnet test TbhReportCatalog.sln` (23 tests), and `dotnet run --project src/Tbh.ReportCatalog -- --month 2025-01` all passed on 2026-03-30. Current dummy DB caveat: `data/command_alkon_dummy.db` does not contain `stxd` or `taxc`, so the emitted CSV is currently header-only on dummy data; live data or a refreshed dummy DB is needed to validate real row output and may require a join-key adjustment once actual STXD/TAXC samples are observed.

### Analytics/report structure cleanup after first live validation pass #nextaction
- [x] #nextaction Redefine `TickValidation_DayPlant.csv` as its own standalone analytic definition instead of reusing the broader `DispatchPlantDay` rollup: group/output should support historical validation directly and include `day`, `plant_code`, `delv_qty_uom`, `load_count`, and `quantity` in a single singular analytic file. — DONE: verified the pipeline now builds `TickValidation_DayPlant` via its own dedicated builder (`DispatchAnalyticsBuilders.BuildTickValidationDayPlant`) rather than reusing `DispatchPlantDay`; the standalone analytic groups by day + plant + delivered UOM and outputs the intended validation columns `day`, `plant_code`, `delv_qty_uom`, `load_count`, and `quantity`. Evidence: `/home/montblanc/repos/tbh-report-catalog/src/Tbh.Analytics/Builders/DispatchAnalyticsBuilders.cs` and `/home/montblanc/repos/tbh-report-catalog/src/Tbh.ReportCatalog/Pipeline/PipelineRunner.cs`; focused coverage in `/home/montblanc/repos/tbh-report-catalog/src/Tbh.ReportCatalog.Tests/Analytics/TickValidationBuilderTests.cs`.
- [x] #nextaction Add a limited-run execution mode so monthly runs can select subsets of analytics and/or reports instead of always running the full pipeline; design the selector surface first, then implement it cleanly through central output IDs rather than scattered ad hoc conditionals. — DONE: verified the selector surface is now implemented centrally via CLI flags `--analytics id1,id2` and `--reports id1,id2`, parsed in `src/Tbh.ReportCatalog/Program.cs` into `RunSelection`, with preset support via `RunSelection.Validation` in `src/Tbh.ReportCatalog/Configuration/RunSelection.cs` and tests covering month/validation/filter behavior in `src/Tbh.ReportCatalog.Tests/Pipeline/ProgramArgumentParsingTests.cs`. Evidence: `dotnet test TbhReportCatalog.sln` passed on 2026-04-22 (33 tests); current repo HEAD `2c90c7e` already contains the implementation.
- [x] #nextaction Audit the existing analytics and reports to enforce the rule that each analytic produces exactly one CSV artifact and each report produces exactly one report artifact; identify and fix places where one output is currently piggybacking on another analytic definition or where a single output concept is blurred across multiple artifacts. — DONE: audited `PipelineRunner`/report wiring and confirmed the main analytics/report path already follows the one-artifact-per-ID contract via `WriteAnalyticAsync`, `CopyLatestAnalytic`, `AddAnalyticArtifact`, and `RunReportAsync`. Found the remaining exception in `src/Tbh.ReportCatalog/Pipeline/GlActivityAnalyticsWriter.cs`, where one helper emits four separate analytics artifacts (`GlActivityLines`, `GlActivity_BySource`, `FastPnL_DepartmentSummary`, `FastPnL_AccountLines`) outside the central catalog path. Captured the concrete follow-up below to migrate those outputs into the same central one-ID/one-artifact contract. Evidence: repo audit on 2026-04-30 covering `src/Tbh.ReportCatalog/Pipeline/PipelineRunner.cs` and `src/Tbh.ReportCatalog/Pipeline/GlActivityAnalyticsWriter.cs`.
- [ ] #nextaction Migrate the legacy GL activity analytics helper into the central analytics catalog so each GL/AP analytic also follows the same one-analytic → one-CSV contract (`GlActivityLines`, `GlActivity_BySource`, `FastPnL_DepartmentSummary`, `FastPnL_AccountLines` should be first-class centrally-wired analytics or be retired/merged explicitly).
...
- [ ] #nextaction Define and document the canonical business definition for the dispatch-ops report family (Ticket Analysis, Rentals, Concrete Truck Time, Driver Production): concrete-only workflow, ticket-grain operational truth, operating plant vs home plant semantics, loads/trips = qualifying tickets, and the rule that `end_unld_time` is not a trustworthy trip-time endpoint for hours.
- [ ] #nextaction Add a follow-up analytics/design todo for other reports that may need `sched_load_time -> load_time` delay analysis or schedule-vs-actual timing metrics; keep that separate from Ticket Analysis trip-time logic so dispatch cycle hours remain strictly `load_time -> at_plant_time`.
- [ ] #nextaction Audit existing reports/analytics that currently use `end_unld_time` (for example the current zone/service timing work) and update them if appropriate so they do not treat `end_unld_time` as a trustworthy primary endpoint for trip/cycle hours; document any places where it remains acceptable as a secondary or report-specific metric.
- [x] #nextaction Extract Command reference/master tables needed for dispatch-ops reporting: `TRUC` (truck assignments) and `EMPL` (employee names/metadata), and confirm whether `TICK.driv_empl_code` should also be carried as the actual ticket-time driver field alongside truck-assigned driver. — DONE (base extraction/normalization): `TRUC` and `EMPL` are already extracted via `ICommandAlkonExtractor` / `SqliteCommandAlkonExtractor` / `AdoCommandAlkonExtractorBase`, normalized via `CommandAlkonMasterDataNormalizer`, and written by `PipelineRunner` as normalized `Trucks.csv` and `Employees.csv`. Remaining follow-on work is specifically to carry and use `TICK.driv_empl_code` in the new Ticket Analysis detail analytic.
- [x] #nextaction Add normalized reference datasets for fleet/people: `NormalizedTrucks.csv` (truck_code, assgn_driv_empl_code, assgn_plant_code, any stable descriptive fields) and `NormalizedEmployees.csv` (empl_code, name, status/useful driver flags if available), with trimming/standardization only. — DONE in substance: normalized fleet/people datasets already exist as `Trucks.csv` and `Employees.csv` with trimming/standardization only. Remaining cleanup is naming/contract alignment if we want the artifact names to become `NormalizedTrucks.csv` / `NormalizedEmployees.csv`.
- [ ] #nextaction Define the plant-group mapping for Ticket Analysis reporting as a first-class auditable rule (not an Excel-only layout hack): at minimum group `{2,6}` together and `{4,5}` together, document naming, and decide how plants `40,41,42,43,44,46` should appear (excluded, included as `OTHER`, or preserved individually with a reporting-group field).

#### Phase 2 — canonical analytic design
- [ ] #nextaction Design the canonical Ticket Analysis source analytic as a single flat, pivot-friendly dataset (likely `TicketAnalysisDetail.csv`) at **ticket grain** rather than line grain unless a specific required sheet proves otherwise; include exact column contract, source datasets, and field definitions before implementation.
- [ ] #nextaction Implement ticket-grain concrete inclusion logic for Ticket Analysis analytics: include only non-removed tickets with reportable concrete quantity; derive ticket-level concrete quantity from qualifying CY ticket lines; explicitly exclude non-concrete/UOM-other activity from this report family instead of relying on Excel filters.
- [ ] #nextaction Decide and document the exact CY/concrete reporting rule for this workflow: whether to preserve raw normalized UOM plus a derived reporting flag/group, or to emit only concrete-qualified rows; make the rule explicit and reusable across Ticket Analysis, Rentals, Concrete Truck Time, and Driver Production.
- [ ] #nextaction Design and implement the ticket-level enrichment joins for the Ticket Analysis analytic: operating plant from ticket/dispatch data, home plant from `TRUC.assgn_plant_code`, assigned driver from `TRUC.assgn_driv_empl_code -> EMPL`, and optionally actual ticket driver from `TICK.driv_empl_code -> EMPL` if present and useful for comparison.
- [ ] #nextaction Add derived operational classification fields to the Ticket Analysis analytic: `operating_plant`, `home_plant`, `reporting_plant_group`, `is_home_plant_run`, and `is_rental` (`home_plant != operating_plant`) so Rentals and split fleet reporting do not depend on manual workbook logic.
- [ ] #nextaction Define the canonical dispatch trip-time / hours metric for this report family: primary truth is `load_time -> at_plant_time`; do **not** use `end_unld_time` as the main endpoint because it is calculated and not trustworthy for trip hours.
- [ ] #nextaction Design the trip-time validity rules and explainable fallback hierarchy for missing/invalid `load_time -> at_plant_time` values: include sanity bounds, rejection rules, and documented imputation order (for example same-order average, then same operating plant, then plant-group/global fallback) before any hours-driven report is implemented.
- [ ] #nextaction Add explicit trip-time provenance fields to the Ticket Analysis analytic so hours are auditable: raw observed minutes, final reportable minutes, source method, imputed flag, and quality flag (`Observed`, `PartiallyImputed`, `FullyImputed`, `Rejected` or similar).
- [ ] #nextaction Build `TicketAnalysisDetail.csv` as the reusable Layer 3 source for the Excel Ticket Analysis workbook, with enough fields to support all known sheets: date, plant/grouping fields, truck, driver name/number, home plant, concrete quantity, load count, trip time/hours, and any helper fields needed for pivots.
- [ ] #nextaction Validate whether the Ticket Analysis workbook should use real Excel PivotTable distinct-count support for unique truck counts or whether a helper analytic is needed; decide this before building the final workbook so `Truck Count (unique trucks)` on By Day Detail is correct and explainable.

#### Phase 3 — workbook contract and report implementation
- [ ] #nextaction Design the workbook/tab contract for `Ticket Analysis Report.xlsx`: include a front `Report Page` plus `Data` source sheet and real PivotTable sheets for at least By Day Detail, Truck By Day, Truck, Day, Driver, and Rentals; record exact filters/rows/values for each tab in repo docs before coding. `Report Page` should capture run parameters, core business-rule notes, and trip-time coverage metrics (total qualifying tickets, observed valid trip time, missing trip time, rejected trip time, and later imputed share once inference exists).
- [ ] #nextaction Implement the By Day Detail pivot design: filters = plant or reporting plant group; rows = date; values = volume, loads, unique truck count; preserve the operational meaning of grouped plants rather than hardcoding ad hoc duplicated pivots.
- [ ] #nextaction Decide the workbook implementation path for `Ticket Analysis Report.xlsx`: true Excel PivotTables if EPPlus can support the required pivot behavior cleanly in this repo, otherwise a first-pass non-pivot workbook that still honors the tab/report-page contract using precomputed tables from `TicketAnalysisDetail` / `TicketAnalysisSummary`.
- [ ] #nextaction Implement the Truck By Day pivot design as a real Excel pivot: filters = concrete/UOM + truck; rows = plant/group then date; values = volume and loads; ensure it remains useful for quickly finding the last day a truck ran.
- [ ] #nextaction Implement the Truck pivot design as a real Excel pivot: filters = concrete/UOM + plant/group; rows = truck; values = quantity and loads.
- [ ] #nextaction Implement the Day pivot design as a real Excel pivot: filters = concrete/UOM + plant/group; rows = date; values = quantity and loads; deliberately solve the “show blank days in the month” requirement via workbook/pivot settings or a date scaffold instead of leaving it as an undocumented Excel trick.
- [ ] #nextaction Implement the Driver pivot design as a real Excel pivot: filters = concrete/UOM + plant/group; rows = driver name (and optionally employee code); values = quantity and loads.
- [ ] #nextaction Implement the Rentals pivot design as a real Excel pivot: filters = concrete/UOM + operating plant/group; rows = home plant then truck; values = quantity, loads, and trip time/hours; ensure the source analytic clearly distinguishes home plant from operating plant.
- [ ] #nextaction Decide whether the Ticket Analysis workbook should preserve the current human-friendly layout of multiple separate pivots per sheet (including grouped plant pairings like `2+6` and `4+5`) or whether a smaller number of pivots with plant-group filters is the better long-term UX; document the chosen presentation standard.

#### Phase 4 — follow-on shared reports and reconciliation
- [ ] #nextaction Define the shared dispatch-ops analytic family for follow-on non-pivot reports so Ticket Analysis can feed `Concrete Truck Time` and `Driver Production` instead of duplicating business logic in separate workbooks.
- [ ] #nextaction Design `Concrete Truck Time` as a report derived from the shared dispatch-ops analytics: one row per truck within plant/reporting group, metrics for Hours, Trips, Yards, and YTD variants, with an explicit decision about whether Hours means observed/imputed dispatch cycle hours rather than payroll hours.
- [ ] #nextaction Design `Driver Production` as a report derived from the shared dispatch-ops analytics: one row per driver (employee code + name), metrics for Yards, Trips, Hours, Yards/Trip, Hours/Trip, Yards/Hour, and both MTD/YTD versions, all tied back to the same canonical hours definition.
- [ ] #nextaction Explicitly separate “dispatch cycle hours” from payroll/labor hours everywhere in docs and outputs so future comparisons to historical reports are honest; if historical reports used a different concept, capture that as a comparison note rather than bending the new pipeline truth to match.
- [ ] #nextaction Add a validation/reconciliation plan for the new dispatch-ops report family: compare new Ticket Analysis / Truck Time / Driver Production outputs against trusted historical reports, record where the new truth intentionally differs, and document accepted differences due to cleaner definitions or timestamp fallback rules.

#### Working plan (2026-04-02)
- Start with **Phase 1** and **Phase 2 design**, not workbook mechanics.
- First concrete deliverable: a written spec covering business rules, plant semantics, trip-time rules, source tables, and the proposed `TicketAnalysisDetail.csv` column contract.
- Repo audit on 2026-04-02 found that the spec foundation already exists in `/home/montblanc/repos/tbh-report-catalog/docs/ticket-analysis-report-design.md`; `TRUC`/`EMPL` extraction + normalization are already wired, and the main remaining implementation gap is the new shared analytic `TicketAnalysisDetail.csv` plus targeted audit of existing timing/driver analytics.
- Only after that spec is stable should implementation begin for new extracts/normalized datasets and the ticket-detail analytic.
