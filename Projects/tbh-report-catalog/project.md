...
- [ ] #nextaction Extract Command reference/master tables needed for dispatch-ops reporting: `TRUC` (truck assignments) and `EMPL` (employee names/metadata), and confirm whether `TICK.driv_empl_code` should also be carried as the actual ticket-time driver field alongside truck-assigned driver.
- [ ] #nextaction Add normalized reference datasets for fleet/people: `NormalizedTrucks.csv` (truck_code, assgn_driv_empl_code, assgn_plant_code, any stable descriptive fields) and `NormalizedEmployees.csv` (empl_code, name, status/useful driver flags if available), with trimming/standardization only.
