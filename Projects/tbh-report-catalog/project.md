...
- [ ] #nextaction Design the workbook/tab contract for `Ticket Analysis Report.xlsx`: `Data` source sheet plus real PivotTable sheets for at least By Day Detail, Truck By Day, Truck, Day, Driver, and Rentals; record exact filters/rows/values for each tab in repo docs before coding.
- [ ] #nextaction Implement the By Day Detail pivot design: filters = concrete/UOM + plant or reporting plant group; rows = date; values = volume, loads, unique truck count; preserve the operational meaning of grouped plants rather than hardcoding ad hoc duplicated pivots.
