...
- [ ] #nextaction Redefine `TickValidation_DayPlant.csv` as its own standalone analytic definition instead of reusing the broader `DispatchPlantDay` rollup: group/output should support historical validation directly and include `day`, `plant_code`, `delv_qty_uom`, `load_count`, and `quantity` in a single singular analytic file.
